...
const videoHistory = [
  {
    id: 2,
    title: "Content Creation Secrets for Beginners",
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg p-6">
        ...
        <p className="text-gray-600 dark:text-slate-400 text-lg">
          Download your personalized AI-generated videos and grow your Social
          Media
        </p>
      </div>
      ...
      <Alert className="border-primary/30 dark:border-teal-500/30 bg-primary/10 dark:bg-teal-500/10 text-primary-dark dark:text-teal-200">
        ...
        <AlertTitle className="font-bold text-gray-900 dark:text-white">
          Your AI Video is Ready!
        </AlertTitle>
        <AlertDescription>
          A new personalized video has been generated based on your selected
          topic.
        </AlertDescription>
      </Alert>
      ...
      <Card className="bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-800">
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Badge
              className={`capitalize ${getStatusClasses(currentVideo.status)}`}
              variant="outline"
            >
              ...
              <div className="flex flex-wrap gap-2">
                {currentVideo.tags.map((tag, index) => (
                  <Badge
                    key={index}
                    variant="outline"
                    className="text-gray-600 border-gray-300 dark:text-slate-400 dark:border-slate-700"
                  >
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        copyToClipboard(currentVideo.title, "Title")
                      }
                    >
                      <Copy className="h-4 w-4" />
                    </Button>
                  </div>
                  <div className="p-3 bg-gray-100 dark:bg-slate-800 rounded-lg text-sm text-gray-800 dark:text-slate-300 border border-gray-200 dark:border-slate-700">
                    {currentVideo.title}
                  </div>
                </div>
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        copyToClipboard(currentVideo.description, "Description")
                      }
                    >
                      <Copy className="h-4 w-4" />
                    </Button>
                  </div>
                  <div className="p-3 bg-gray-100 dark:bg-slate-800 rounded-lg text-sm text-gray-600 dark:text-slate-400 max-h-32 overflow-y-auto border border-gray-200 dark:border-slate-700">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="mt-1 p-2 bg-gray-100 dark:bg-slate-800 rounded text-sm text-gray-800 dark:text-slate-300 border border-gray-200 dark:border-slate-700">
                      {currentVideo.topic}
                    </div>
                  </div>
                  <div>
              ...
              <div className="space-y-3 pt-4 border-t border-gray-200 dark:border-slate-800">
                <Button
                  ...
                  className="w-full bg-primary hover:bg-primary-hover text-primary-foreground dark:bg-teal-600 dark:hover:bg-teal-700 dark:text-white py-3 text-base font-semibold"
                >
                  {isDownloading ? (
                    <>
                ...
                <Button
                  variant="outline"
                  className="w-full border-gray-300 hover:bg-gray-100 dark:border-slate-700 dark:hover:bg-slate-800"
                  asChild
                >
                  <a
                    href="/tutorials/upload-guide"
                    target="_blank"
                    rel="noreferrer"
                  >
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {videoHistory.map((video) => (
              <Card
                key={video.id}
                className="bg-gray-50 dark:bg-slate-800/50 border-gray-200 dark:border-slate-700 hover:border-gray-300 dark:hover:border-slate-600 transition-colors"
              >
                <div className="relative mb-3">
                  ...
                  <div className="absolute top-2 right-2">
                    <Badge
                      className={`capitalize ${getStatusClasses(video.status)}`}
                      variant="outline"
                    >
                      {video.status}
                    </Badge>
                ...
                <div className="p-4 pt-0">
                  <h4 className="font-medium text-sm text-gray-800 dark:text-slate-200 mb-2 line-clamp-2">
                    {video.title}
                  </h4>
